...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    # If both expr1 and exp2 are sums
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        total = []
        # go through each number getting summed in both expressions and multiply each by eachother
        for num1 in expr1:
            for num2 in expr2:
                # append as new Product to list of Products
                total.append(Product([num1, num2]))
        # return sum of all products
        return Sum(total).simplify()

    # If expr1 is sum and exp2 is product
    elif isinstance(expr1, Sum) and isinstance(expr2, Product):
        total = []
        # go through each number getting summed
        for num in expr1:
            # multiply each number getting summed by the full Product and append new product to total, to be summed
            total.append(Product([num, expr2]))
        # return sum of all products
        return Sum(total).simplify()

    # if expr1 is product and expr2 is sum
    elif isinstance(expr1, Product) and isinstance(expr2, Sum):
        total = []
        # go through each number getting summed
        for num in expr2:
            # multiply each number getting summed by the full Product and append new product to total, to be summed
            total.append(Product([num, expr1]))
        # return sum of all products
        return Sum(total).simplify()

    # if both expr 1 and expr2 are products
