    ...
    def count_pieces(self, current_player=None) :
        ...
        if current_player not in [True, False, None]:
            raise TypeError("Expected boolean value for current_player, got "
                            + str(current_player))
        player = self.get_current_player_name() if current_player else self.get_other_player_name()
        player_test = (lambda x : x) if current_player is None else (lambda x : x == player)
